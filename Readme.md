...
├── services/
│   └── employeeApi.ts             # Axios API calls
├── types/
...
- **Same field, multiple filters** → OR logic (record matches any condition)
...
- **Mock API** — Express server serves `employees.json` so the app behaves like a real API integration
...
| `npm run mock` | Start Express mock API |
