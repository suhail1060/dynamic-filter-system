  ...
  return (
    <Box sx={{
      ...
      <FormControl size="small" sx={{ minWidth: 160 }}>
        <Select value={filter.fieldKey} onChange={e => handleFieldChange(e.target.value)} sx={selectSx}>
          {FIELD_DEFINITIONS.map(f => (
            <MenuItem key={f.key} value={f.key} sx={{ fontSize: 13 }}>{f.label}</MenuItem>
          ))}
        </Select>
      </FormControl>
      ...
      {operators.length > 0 && (
        <FormControl size="small" sx={{ minWidth: 160 }}>
          <Select value={filter.operator} onChange={e => onUpdate({ ...filter, operator: e.target.value as OperatorType })} sx={selectSx}>
            {operators.map(op => (
              <MenuItem key={op.value} value={op.value} sx={{ fontSize: 13 }}>{op.label}</MenuItem>
            ))}
          </Select>
        </FormControl>
      )}
